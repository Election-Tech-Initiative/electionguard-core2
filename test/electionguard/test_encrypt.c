...
static bool test_encrypt_ballot_simple_cast_removes_nonces(void);
...
bool test_encrypt_ballot_simple_cast_removes_nonces(void)
{
    ...
    if (eg_ciphertext_ballot_cast(ciphertext)) {
        assert(false);
    }

    // Assert
    eg_element_mod_q_t *nonce_ref = NULL;
    if (eg_ciphertext_ballot_get_nonce(ciphertext, &nonce_ref)) {
        assert(false);
    }

    assert(nonce_ref == NULL);
